...
The nodes in a \ac{mcts} tree represent a state in the underlying model and the edges correspond to transitions between these states.
...
\end{equation}
We choose to have a sequence of history instructions $h$ with a defined length $n$.
...
For example, a instruction $a$ depends on a instruction $b$ and we are in the situation in which we want to schedule $a$.
...
To build a dataset with these mappings we iterate the \ac{mcts} trees and for every instruction node, we take its past scheduled instructions and the reward value it has.
...
We transform the instructions from their textual representation into a ordinal encoding, \ie every instruction gets a unique numerical representation.
...
With this data we train a \ac{svm} regression model.
...
With hthis mode, we try to learn the same mapping as we did with our \ac{svm} model, which is defined in \cref{eqn:approach:regression-mapping}.
Consequently, we use the same dataset that we have already built for the \ac{svm} model.

In contrast to our \ac{svm} model, we chose to not encode the instructions into ordinary values.
...
To reduce the dimensionality of the data, we clustered the similar instructions.
Especially instructions that only vary in the number of bits in the inputs.
The instructions addition instruction \lstinline|ADDWri| and \lstinline|ADDXri| of the AARCH64 architecture are clustered into the same cluster.
These two instruction only differ in that the one takes 32 bit values and the other 64 bit values.
...
\todo{Write something about how we run the learned models and how we make measurements}
