...
Lastly, we give a brief overview of the thesis's machine learning or data-driven approaches in \Cref{sec:bg:ml}.
...
\begin{enumerate}
    ...
    \item Decode:
        ...
        Further, the \ac{cpu} computes actual memory addresses from base addresses and offsets.
    \item Execution:
        Here, the instruction is executed with the given operands on the respective processing unit.
    \item Writeback:
        This phase writes back the computed result to the specified register.
\end{enumerate}
...
\Cref{tbl:bg:schedule-a} defines the instruction schedule for this example.
...
There are various reasons of hazards that cause these stalls in the pipeline.
\begin{itemize}
    ...
    \item Data Hazards:
        There are three types of different data hazards when two instructions attempt to access the same register:
        \begin{itemize}
    ...
    \item Control Hazards:
        ...
        Usually, the \ac{cpu} executes one branch speculatively and abort the instructions if it is the wrong branch.
...
There are various ways to implement superscalar pipelines.
...
\begin{itemize}
    ...
    \item Very Long Instruction Word: 
        The compiler can define which instructions the CPU should execute simultaneously for this type of superscalar pipeline.
...
\Cref{tbl:bg:schedules} defines these two schedules and we compare them in this section.
...
However, the overall execution is with 15 cycles slower than the 10 cycles of the execution in \Cref{tbl:bg:schedule-comparison-a} because the \ac{cpu} stalls more often.
...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance=2cm]
        ...
        \node (optimizer) [defaultnode,right=4.5cm-0.6pt of rust_front_end] {Optimizer};

        \node (ve_backend) [defaultnode,right=4.5cm-0.6pt of optimizer] {\aurora{} \\ Back-End};
...
Eventually, the source code is translated into an \ac{ir} which is then used as input to the optimizer and the back-end.
...
Further, it also decides where the processor stores variables (registers only or memory).
...
When we look at the superscalar pipelines (see \Cref{sec:bg:superscalar-cpu}), the out-of-order pipeline would benefit the least from better instruction schedules because it can reschedule the instructions themselves.
...
That makes them faster, but it also becomes harder to predict what they would do in a given situation, \ie to predict how long it would take to execute a given instruction.
...
However, there is research on extending the scope of instruction scheduling (see \Cref{sec:rw:instruction-scheduling}).
...
% How
In \Cref{fig:bg:example-dag} we see that before we can execute the \iloc{store} instruction $i_9$ to move the result back to memory, we have to run the last \iloc{add} instruction $i_8$.
This \iloc{add} instruction $i_8$ depends on another \iloc{add} $i_7$ and a \iloc{mult} instruction $i_6$.
...
The processors have a defined set of different classes of registers, \eg there are often different registers for integer and floating-point numbers.
...
\begin{table}
    ...
    \begin{tabular}{@{}rllcl@{}} \toprule
        ...
        \bottomrule
    \end{tabular}
    \caption[Instruction Schedule with Reduced Register Pressure]{Instruction schedule with reduced register pressure}
...
That means we can run LLVM front-ends like \iloc{clang}, or \iloc{rustc} to generate LLVM \ac{ir} code.
...
The arrows represent the dependencies between instructions.
They point from a given instruction to one of its dependency instructions.
There are three different types of edges that represent different types of dependencies:
\begin{itemize}
...
Additionally, there is the \lstinline|GraphRoot| node at the bottom, which does not represent any instruction and does not get scheduled.
...
When all decisions in a level were seen at least once, the algorithm balances exploitation and exploration, which means it chooses the best possible decision most of the time.
However, sometimes it chooses another decision to learn more in an undiscovered part of the decision tree.
This balancing causes \ac{mcts} to learn how to extend the paths with better decisions.

We explain the algorithm in more detail here.
...
\begin{enumerate}
    ...
    \item Simulation Phase:
        From the selected node, the algorithm fills the path through the \ac{mcts} tree with randomly chosen nodes until a terminal state is reached.
        In this phase, no nodes are added to the tree.
        The randomly selected nodes are only used to fill the path.
...
We use the \ac{svr} model, but we first briefly explain the classification model, as the regression model is based on this.
...
\Cref{fig:bg:nn-example} visualizes an example neural network.
\begin{figure}
    \centering
